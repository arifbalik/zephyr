...
void stm32_tsc_start(const struct device *dev)
{
	...
	/* TODO: When sync acqusition mode is enabled, both this bit and an external input signal should be set.
	 * When the acqusition stops this bit is cleared, so even if a sync signal is present, the next acqusition will not start
...
#define STM32_TSC_INIT(index)                                                                                                \
	...
	{                                                                                                                    \
		IRQ_CONNECT(DT_INST_IRQN(index), DT_INST_IRQ(index, priority), stm32_tsc_isr, DEVICE_DT_INST_GET(index), 0); \
	...
	DEVICE_DT_INST_DEFINE(index, stm32_tsc_init, NULL, NULL, &stm32_tsc_cfg_##index, POST_KERNEL, CONFIG_KERNEL_INIT_PRIORITY_DEFAULT, NULL);
